    ...
    class NPCTank : BasicModel
    {
        ...
        public NPCTank(Vector3 startPosition, Model model, Player player) : base(startPosition, ObjectManager.Camera, model)
        {
            ...
            if (loadFromFile) {
                ...
                foreach (XElement stateElement in xdoc.Root.Elements()) {
                    Console.WriteLine(stateElement.Attribute("state"));
                    AiState state = new AiState(brain, stateElement.Attribute("state").Value);
                    foreach (XElement transitionElement in stateElement.Elements()) {
                        Console.WriteLine(transitionElement);
            ...
            else
            {
                ...
                AiState PatrolState = new AiState(brain, "PATROL");
                PatrolState.addTransition("SEEK", playerNear);
                AiState SeekState = new AiState(brain, "SEEK");
                SeekState.addTransition("IDLE", playerFar);
                AiState IdleState = new AiState(brain, "IDLE");
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            switch (brain.CurrentState) {
                case "PATROL": updatePatrol(deltaTime); break;
                case "SEEK": updateSeek(deltaTime); break;
        ...
        public override void Draw(GameTime gameTime)
        {
            if (pathPoints != null && (brain.CurrentState.Equals("PATROL") || brain.CurrentState.Equals("SEEK")))
                if (pathPoints.Count() > 1)
                    PathRenderer.RenderPath(pathPoints, Color.Yellow, 0);
            base.Draw(gameTime);
        }
        ...
        /// </summary>
        /// <param name="deltaTime"></param>
        private void updateSeek(float deltaTime) {
